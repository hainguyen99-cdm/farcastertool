...
const ScriptExecutor: React.FC<ScriptExecutorProps> = ({ 
  ...
  return (
    <>
      <button
        type="button"
        className="rounded-md border border-green-200 bg-white px-2.5 py-1.5 text-xs font-medium text-green-700 hover:bg-green-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-green-600 focus-visible:ring-offset-2"
        onClick={handleOpen}
        disabled={isExecuting}
        aria-label={`Run script for ${accountName}`}
      >
        Run Script
      </button>

      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="script-executor-title">
          <div className="fixed inset-0 bg-black/40" onClick={handleClose} />
          <div className="relative z-10 w-full max-w-2xl rounded-lg bg-white p-6 shadow-xl ring-1 ring-gray-200">
            <div className="mb-4 flex items-start justify-between">
              <h2 id="script-executor-title" className="text-lg font-semibold">Run Script - {accountName}</h2>
              <button
                type="button"
                onClick={handleClose}
                ...
                aria-label="Close script executor"
              >
                ✕
              </button>
            </div>
            ...
            <div className="mb-4 space-y-3">
              ...
              {actions.length === 0 && (
                <div className="rounded-lg border border-dashed border-gray-300 bg-gray-50 px-4 py-8 text-center">
                  <p className="text-sm text-gray-500">No actions added yet. Add actions to create a script.</p>
                </div>
              )}
            </div>
