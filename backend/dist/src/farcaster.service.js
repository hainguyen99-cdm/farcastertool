...
let FarcasterService = class FarcasterService {
    ...
    async uploadMediaFile(uploadUrl, fileBuffer, fileName, mimeType = 'image/jpeg') {
        try {
            ...
            const response = await this.executeWithRetry(async () => (0, rxjs_1.firstValueFrom)(this.httpService.post(uploadUrl, formData, {
                headers: {
                    ...formData.getHeaders(),
                    'accept': '*/*',
                    'accept-language': 'en-US,en;q=0.9',
            ...
            throw new common_1.HttpException('Failed to upload media file', this.resolveStatus(err));
        }
    }
    async createCast(encryptedToken, text, embeds) {
        ...
        try {
            ...
            const response = await this.executeWithRetry(async () => (0, rxjs_1.firstValueFrom)(this.httpService.post(`${this.baseUrl}/v2/casts`, payload, {
                headers: {
                    ...this.buildAuthHeaders(token),
                    'accept': '*/*',
                    'accept-language': 'en-US,en;q=0.9',
